...
export function subscribeToProjectRealtime(projectId: string, onChange: (change?: RealtimeChange) => void): RealtimeUnsubscribe {
  const channel = supabase
    ...
    .on(
      'postgres_changes',
      { event: '*', schema: 'public', table: 'cron_job_patch_requests', filter: `project_id=eq.${projectId}` },
      (payload) =>
        onChange({
          table: 'cron_job_patch_requests',
          event: (payload as any).eventType,
          new: (payload as any).new,
          old: (payload as any).old,
        })
    ...
    .on(
      'postgres_changes',
      { event: '*', schema: 'public', table: 'cron_create_requests', filter: `project_id=eq.${projectId}` },
      (payload) =>
        onChange({
          table: 'cron_create_requests',
          event: (payload as any).eventType,
          new: (payload as any).new,
          old: (payload as any).old,
        })
    ...
    .on(
      'postgres_changes',
      { event: '*', schema: 'public', table: 'skills_mirror', filter: `project_id=eq.${projectId}` },
      (payload) =>
        onChange({
          table: 'skills_mirror',
          event: (payload as any).eventType,
    ...
    .on(
      'postgres_changes',
      { event: '*', schema: 'public', table: 'chat_delivery_queue', filter: `project_id=eq.${projectId}` },
      (payload) =>
        onChange({
          table: 'chat_delivery_queue',
          event: (payload as any).eventType,
          new: (payload as any).new,
          old: (payload as any).old,
        })
