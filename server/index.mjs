  ...
  try {
    ...
    if (req.method === 'POST' && url.pathname === '/api/agents/provision') {
      try {
        ...
        const agentsContent = `# AGENTS.md\n\nThis file is a pointer to the operator rulebook and project rules.\n\n- Follow the project’s Mission + Overview + pinned rulebook docs.\n- Follow the global operator handbook in the repo: docs/front-office/OPENCLAW-ANATOMY-CHEATSHEET.md and related Front Office docs.\n\nPreflight (before asking the operator):\n- Check pinned docs + Knowledge search\n- Check TOOLS.md for environment/tool notes\n- Check if credentials/sessions already exist\n\nAfter meaningful work:\n- Post a completion summary + links in the task timeline\n`;
        const memoryContent = `# MEMORY.md\n\n`;
        const skillsContent = `# SKILLS.md\n\nThis is a lightweight pointer file.\n- Prefer the canonical skills cheat sheet in the repo: docs/front-office/SKILLS-CATALOG.md\n- If you need a new playbook, propose a task: \"Skill/Playbook needed: <name>\"\n`;

        const seedFiles = [
          { fp: path.join(workspaceDir, 'SOUL.md'), content: soulContent },
          { fp: path.join(workspaceDir, 'IDENTITY.md'), content: identityContent },
          { fp: path.join(workspaceDir, 'USER.md'), content: userContent },
          { fp: path.join(workspaceDir, 'AGENTS.md'), content: agentsContent },
          { fp: path.join(workspaceDir, 'TOOLS.md'), content: '# TOOLS.md\n\n(Operator/environment notes live here. Keep it short.)\n' },
          { fp: path.join(workspaceDir, 'MEMORY.md'), content: memoryContent },
          { fp: path.join(workspaceDir, 'SKILLS.md'), content: skillsContent },
        ];
